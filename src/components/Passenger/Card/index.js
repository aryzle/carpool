...
const handleDelete = (personId, eventId) => () =>
  firebase.database().ref(`events/${eventId}/persons/${personId}`).remove()

const PassengerCard = ({ passenger, eventId }) => {
  const {name, email, address, city, state, earliestDepartureDateTime, latestReturnDateTime, info } = passenger
  ...
  return (
    <Card>
      ...
      <Card.Content extra>
        <Icon name='user' />
        {info}
        <Icon color="red" name='trash outline' style={{float: "right"}} onClick={handleDelete(passenger.id, eventId)} link />
      </Card.Content>
    </Card>
  )
}
