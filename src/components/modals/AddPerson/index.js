...
export default class AddPerson extends Component {
  ...
  render() {
    ...
    const { name, email, phone, city, address, state, earliestDepartureDateTime,
      latestReturnDateTime, info, success, error } = this.state
      ...
      <Modal trigger={trigger || <Button inverted color="blue">Need a ride?</Button>}>
        ...
        <Modal.Content form>
          <Form onSubmit={this.handleSubmit} size="small" success={success}
            ...
            <Form.Input required placeholder="Email*" name="email" value={email}
                        onChange={this.handleChange} />
            ...
            <Form.Input placeholder="Address" name="address" value={address}
                        onChange={this.handleChange} />
            <Form.Select required placeholder="State" name="state" value={state}
                         options={stateOptions} onChange={this.handleChange} />
            <Form.Field required>
              <label>When is the earliest you could leave?</label>
              <DatePicker onChange={this.handleDepDateChange}
                          selected={earliestDepartureDateTime}
                          shouldCloseOnSelect={false} timeIntervals={15}
                          dateFormat="LLL" showTimeSelect />
            </Form.Field>
            <Form.Field required>
              <label>When is the latest you want to be back by?</label>
              <DatePicker onChange={this.handleRetDateChange}
            ...
            <Form.TextArea
              ...
              name="info" value={info} onChange={this.handleChange} />
